dataset_type = 'UCASDataset'
data_root = '/cluster/nvme9/wjs/UCAS_AOD/data_split/'
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        xmltype='obb',
        imgset=data_root + 'split/train.txt',
        ann_file=data_root + 'Train/annotations_xml/',
        img_prefix=data_root + 'train_jpg/',
        pipeline=train_pipeline),
    test=dict(
        type=dataset_type,
        xmltype='obb',
        imgset=data_root + 'split/test.txt',
        ann_file=data_root + 'Test/annotations_xml/',
        img_prefix=data_root + 'test_jpg/',
        pipeline=test_pipeline))
evaluation = None
